...
local function is_locked(chat_id)
  	...
  	local current = db:hget(hash, 'salvar')
  	if current == 'off' then
  		return true
  	else
  		return false
	...
	if blocks[1] == 'salvar' then
	    ...
	    if msg.reply and not blocks[3] then
	    	...
	    	if not file_id then
	    		...
	    		db:hset('chat:'..msg.chat.id..':salvar', blocks[2], to_save)
	    		api.sendReply(msg, _("This media has been saved as a response to %s"):format(blocks[2]))
	    	end
		else
	    	local hash = 'chat:'..msg.chat.id..':salvar'
	    	local new_salvar = blocks[3]
	    	local reply_markup, test_text = u.reply_markup_from_text(new_salvar)
