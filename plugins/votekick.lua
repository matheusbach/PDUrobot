...
local function do_keyboard_vote(user_id)
	return {
		inline_keyboard = {
			{
				{text = _("☑ Yes"), callback_data = string.format('votekick:increase:%d', user_id)},
...
local function conclusion(initiator, defendant, supports, oppositionists, quorum, upshot, informative)
	...
	if upshot == 'bot not admin' then
		table.insert(lines, _("⚠ The voting was closed, a community decided kick %s, "
			.. "but error ocured while kick this user, because I ceased to be "
			.. "_an admin_. The results:\n"):format(defendant_name))
	end
	if upshot == 'already admin' then
		table.insert(lines, _("⚠ The voting was closed, a community decided kick %s, "
			.. "but error ocured while kick this user, because he became "
			.. "_an admin_. The results:\n"):format(defendant_name))
	end
	if upshot == 'was protected' then
		table.insert(lines, _("The voting was closed. %s suggested to kick %s, but "
...
local function generate_poll(msg, defendant)
	...
	if previous_id then
		...
		if res.result.chat.username then
			...
			text = _("⬇ The poll for kick of %s was closed because [new poll](%s) was created")
				:format(u.full_name(defendant), link)
		else
			text = _("⬇ The poll for kick of %s was closed because *new poll* was created")
				:format(u.full_name(defendant))
		end
...
local function rebuild_poll_message(chat_id, user_id, problems)
	local hash = string.format('chat:%d:votekick:%d', chat_id, user_id)
...
local function cast_vote(chat_id, defendant_id, voter_id, value)
	local hash = string.format('chat:%d:votekick:%d', chat_id, defendant_id)
	if value > 0 then
	...
	if cast_vote(chat_id, user_id, from_id, value) then
		...
		if supports + oppositionists >= quorum then
			...
			if not informative and supports > oppositionists then
				send_confirmation, code = api.kickUser(chat_id, user_id)
			end
...
function plugin.cron()
	...
	for i, hash in pairs(db:keys('chat:*:votekick:*')) do
		local chat_id, user_id = hash:match('chat:(-?%d+):votekick:(-?%d+)$')
		...
		if not ok then
			print(traceback)
			api.sendAdmin('An #error occurred (votekick).\n'..traceback)
			db:del(hash, hash .. ':supports', hash .. ':oppositionists')
		end
		::continue::
	end
...
function plugin.onTextMessage(msg, blocks)
	if blocks[1] == 'votekick' then
		local hash = string.format('chat:%d:settings', msg.chat.id)
		local status = db:hget(hash, 'votekick') or config.chat_settings.settings.votekick
		...
		elseif tonumber(blocks[2]) then
			...
			-- theoretically we can vote for kick of left users
...
function plugin.onCallbackQuery(msg, blocks)
	...
	if blocks[1] == 'cancel' then
		local hash = string.format('chat:%d:votekick:%d', msg.chat.id, defendant)
		local initiator = tonumber(db:hget(hash, 'initiator'))
